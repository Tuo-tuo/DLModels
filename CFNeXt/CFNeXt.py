...
class CFNeXtParas(object):
    ...
    stem_conv2d_num: int = 1
    stem_output_channel: int = 128 # Can be changed.
    stem_channels: list = [input_channel] + [stem_output_channel]*stem_conv2d_num
    stem_data_format: str = 'channels_first'

    stem_output_shapes: list = list() # [B, C, H, W] -> [B, C, H/4, W/4]
    for idx in range(stem_conv2d_num):
        # 2**(idx+2), because the stem_kernel=4, and stem_stride =4.
        stem_output_shapes.append([stem_channels[idx+1], input_h//(2**(idx+2)), input_w//(2**(idx+2))])
    ...
    block_data_format: str = 'channels_first'
    ...
    stage_paras = {
                    ...
                    'ds1_in_channels':s1_ds1_in_channels, 'ds1_out_channels':s1_ds1_out_channels, 'norm_before_ds1_data_format':'channels_first', 
                    'n2':1, 'b2_in_channels':s1_b2_in_channels, 'b2_out_channels':s1_b2_out_channels, 
                    'ds2_in_channels':s1_ds2_in_channels, 'ds2_out_channels':s1_ds2_out_channels, 'norm_before_ds2_data_format':'channels_first', 
...
class Stage(nn.Module):
    def __init__(self, stage_paras: Dict) -> None:
        ...
        self.norm_before_ds1 = LayerNorm(stage_paras['ds1_in_channels'], data_format=stage_paras['norm_before_ds1_data_format']) #
        ...
        self.norm_before_ds2 = LayerNorm(stage_paras['ds2_in_channels'], data_format=stage_paras['norm_before_ds2_data_format']) #
...
class CFNeXt(nn.Module):
    def __init__(self, net_paras) -> None:
        ...
        self.norm_before_ds = LayerNorm(self.net_paras.downsample_in_channels, data_format=self.net_paras.block_data_format)
    ...
    def _create_stem(self) -> nn.Sequential:
        ...
        for idx in range(self.net_paras.stem_conv2d_num):
            layers.append(nn.Conv2d(self.net_paras.stem_channels[idx], self.net_paras.stem_channels[idx+1], 
                                    kernel_size=self.net_paras.stem_kernel, stride=self.net_paras.stem_stride))
            layers.append(LayerNorm(self.net_paras.stem_channels[idx+1], eps=1e-6, data_format=self.net_paras.stem_data_format))
    ...
    def _create_output(self, in_channels: int, out_channels: int) -> nn.Sequential:
        ...
        # layers.append(LayerNorm(self.net_paras.stem_output_channel//2, data_format = 'channels_first'))
        ...
        # layers.append(LayerNorm(self.net_paras.stem_output_channel//4, data_format = 'channels_first'))
        layers.append(nn.GELU())
        ...
        # layers.append(LayerNorm(self.net_paras.stem_output_channel//8, data_format = 'channels_first'))
        layers.append(nn.GELU())

        layers.append(nn.Conv2d(self.net_paras.stem_output_channel//8, out_channels, 1))
        layers.append(nn.BatchNorm2d(out_channels))
        # layers.append(LayerNorm(out_channels, data_format = 'channels_first'))
